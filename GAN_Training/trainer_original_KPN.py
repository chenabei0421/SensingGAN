...
def Pre_train(opt):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (true_input, true_target) in enumerate(train_loader):
            ...
            #if (epoch==0) and (i==112 or i%20==0):  #rain100H_simpletest
            if ((epoch + 1) % 1 == 0) and (i==112 or i%20==0):  #rain100H
            #if (epoch==249) and (i==12 or (i+1)%5==0):  #rain100L
                ...
                #plt.show()
                plt.tight_layout()
                plt.savefig('samples/hist/distribution_e'+ str(epoch) +'_b' + str(i) +'.png')
                fig.clear()
                
                # plot filter channel in grayscale
                plt.imshow(feature_map_hist[0][0][0][0].cpu().detach().numpy(), cmap='gray')
                # save the feature map
                print('save the feature map e'+ str(epoch) +'_b' + str(i) + '.png')
                plt.savefig('samples/featuremap/featuremap_e'+ str(epoch) +'_b' + str(i) + '.png')
