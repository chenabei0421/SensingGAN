...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        # 6~8层要先上采样再卷积
        self.conv6 = TransformerUp(512, 256)
        self.conv7 = TransformerUp(256, 128)
        self.outc = nn.Conv2d(128, out_channel, 1, 1, 0)
    ...
    def forward(self, data_x, data, white_level=1.0):
        ...
        sa = self.MHSA(conv3)  #self-attention
        # 开始上采样  同时要进行skip connection
        conv6 = self.conv6(sa, conv2)
        conv7 = self.conv7(conv6, conv1)
        # return channel K*K*N
        core = self.outc(F.interpolate(conv7, scale_factor=2, mode=self.upMode))
