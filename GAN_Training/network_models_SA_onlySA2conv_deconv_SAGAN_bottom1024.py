...
class Up(nn.Module):
    ...
    def __init__(self, channels_x, channels_y, channels_out):
        ...
        self.tconv = TripleConv(channels_x+channels_y, channels_out)

    def forward(self, x, y):
        deconv_y = self.deconv(y)
        cont_xy = torch.cat([x, deconv_y], dim=1)
        tconvxy = self.tconv(cont_xy)
        return tconvxy
...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.SA = Self_Attn(1024, 'relu')  #self-attention
        # 6~8层要先上采样再卷积
        self.conv6 = Up(512, 1024, 512)
