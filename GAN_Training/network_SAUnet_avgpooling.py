...
class Down(nn.Module):
    ...
    def forward(self, x):
        return F.avg_pool2d(self.tconv(x), 2, 2)
...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.conv8 = TransformerUp(128, 64)
        self.outc = nn.Conv2d(64, out_channel, 1, 1, 0)
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        conv6 = self.conv6(sa, conv3)
        conv7 = self.conv7(conv6, conv2)
        conv8 = self.conv8(conv7, conv1)
