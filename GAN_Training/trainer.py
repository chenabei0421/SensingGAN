...
def Pre_train(opt):
    ...
    # Optimizers
    optimizer_G = torch.optim.Adam(filter(lambda p: p.requires_grad, generator.parameters()), lr = opt.lr, betas = (opt.b1, opt.b2), weight_decay = opt.weight_decay)
    optimizer_D = torch.optim.Adam(filter(lambda p: p.requires_grad, discriminator.parameters()), lr = opt.lr, betas = (opt.b1, opt.b2), weight_decay = opt.weight_decay)
    ...
    for epoch in range(opt.epochs):
        for i, (true_input, true_target) in enumerate(train_loader):
            ...
            if (epoch==9 or epoch==(opt.epochs-1)) and (i%20==0):
                '''
            ...
            generator_loss = -0.01*gan_loss + Pixellevel_L1_Loss + 0.2*ssim_loss + SA_perceptual_loss
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [Loss: %.4f %.4f %.4f %.4f %.4f] Time_left: %s" %
                (epoch, opt.epochs, i, len(train_loader), gan_loss, Pixellevel_L1_Loss.item(), ssim_loss.item(), SA_perceptual_loss, discriminator_loss, time_left))
            ...
            adjust_learning_rate(opt, epoch, optimizer_D)
